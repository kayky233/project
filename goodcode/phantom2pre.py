...
batch_size = 30
epochs = 20
filename = '/home/kayky/project/POI motion data tracked with  TPS model/Phantom-II.mat'
footer = 3
look_back=10
...
if __name__ == '__main__':
    ...
    features = np.reshape(features_struct['SquPOI'], (3389,3), order='F')
    data = pd.DataFrame(features) 
   
    datas = data.values.astype('float32')
    ss = StandardScaler()
    cps = ss.fit_transform(datas)
    datase = pd.DataFrame(cps.reshape(1, 10167, order='F'))
